...
| `ctx.jailbreakMode` | `core` | [`jailbreak-mode`](../packages/jailbreak/jailbreak-mode) | - | - | - | Folds logged jailbreak/mode state, flushes user selections at turn boundaries, renders the strategy system block and message wrappers, scaffolds TVD harnesses, and registers /jailbreak. |
...
| `ctx.collabStudio` | `core` | [`collab-studio`](../packages/web/collab-studio) | - | - | - | Owns persisted multi-role collaboration projects, stage execution, generated files, and the `/api/collab` web surface. |
| `ctx.modelBench` | `core` | [`model-bench`](../packages/web/model-bench) | - | - | - | Owns persisted benchmark rounds, question generation, concurrent contestant runs, judge results, and the `/api/bench` web surface. |
