...
flowchart LR
  ...
  pkg_collab_studio["collab-studio"]
  svc_collabStudio["ctx.collabStudio<br/>AI collaboration studio service"]
  pkg_model_bench["model-bench"]
  svc_modelBench["ctx.modelBench<br/>Model benchmark service"]
  ...
  pkg_collab_studio --> svc_collabStudio
  ...
  pkg_model_bench --> svc_modelBench
...
| `ctx.collabStudio` | `core` | [`collab-studio`](../packages/web/collab-studio) | - | - | - | 拥有持久化的多角色协作项目、阶段执行、生成文件以及 `/api/collab` Web 界面。 |
| `ctx.modelBench` | `core` | [`model-bench`](../packages/web/model-bench) | - | - | - | 拥有持久化的评测轮次、问题生成、并发参赛模型运行、裁判结果以及 `/api/bench` Web 界面。 |
