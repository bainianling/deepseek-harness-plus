...
## Work scope: fix the other harness only

**User directive (2026-08-28).** This agent works only on identifying and fixing issues of the customized local-build harness in this checkout — the instance serving the Web GUI at port 3080 (its client bundle carries the local mods: wallpaper, avatar, LAN sharing, sidebar work). Do not take on feature work or other tasks unrelated to fixing that harness. A separate stock harness serves as the clean reference implementation on port 3081; inspect it only as the reference while fixing the customized one.

## Knowledge prerequisite

Before any non-trivial task, call `hindsight_list_knowledge_pages`, read every relevant page with `hindsight_read_knowledge_page`, and use `hindsight_search_knowledge_pages` when the accumulated project knowledge may answer the question. Do this before re-deriving architecture, conventions, component ownership, or prior decisions from source. If a verified source contradicts memory, record a correction with `hindsight_ingest_document` so later tasks receive the current fact.
