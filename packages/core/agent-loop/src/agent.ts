...
import type { GenerateOptions, LlmCallConfig, LlmResponseLifecycle, Message, PreparedLlmCall } from '@deepseek-ai/dsh-llm'
import {
  BlockAssembler,
  LlmError,
...
/** Persist one adapter-produced response lifecycle in the Session log order. */
function appendResponseReplay(session: Session, lifecycle: LlmResponseLifecycle): void {
  session.append('llm/response-opened', lifecycle.opened)
  for (const item of lifecycle.items) session.append('llm/response-item', item)
  session.append('llm/response-closed', lifecycle.closed)
}
  ...
  private async step(assembly: PromptAssembly, startsRequestSeries: boolean): Promise<StepEndReason | null> {
    ...
    while (true) {
      ...
      try {
        ...
        if (assembler.responseReplay !== undefined) appendResponseReplay(this.session, assembler.responseReplay)
        if (signal.aborted) {
