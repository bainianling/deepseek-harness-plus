...
describe('agent loop', () => {
  ...
  it('persists an adapter response lifecycle beside the assistant turn', async () => {
    const responseReplay = {
      ...
      items: [{
        ...
        payload: { type: 'output_text', text: 'abc' },
      }],
      closed: {
        route: { provider: 'mock', model: 'mock' },
    ...
    const adapter = new MockAdapter([[
      ...
      { type: 'block-end', index: 0, block: { type: 'text', text: 'abc' } },
      { type: 'finish', reason: { kind: 'stop' }, responseReplay },
    ]])
    const ctx = await harness(adapter)
    const agent = await ctx.agentLoop.create(SessionId('response-replay'), { provider: 'mock', model: 'mock' })

    send(agent, 'persist the response')
    await waitForIdle(ctx, agent)

    const lifecycle = agent.session.snapshotEvents()
      .filter(event => event.type.startsWith('llm/response-'))
    expect(lifecycle.map(event => event.type)).toEqual([
      'llm/response-opened', 'llm/response-item', 'llm/response-closed',
    ])
    expect(lifecycle[1]?.data).toEqual(responseReplay.items[0])
  })
