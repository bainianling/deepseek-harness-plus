...
/** Keep the deployment persona prefix as the first equal-order section, then canonicalize ties by name. */
function comparePromptSections(a: PromptSection, b: PromptSection): number {
  const byOrder = a.order - b.order
  if (byOrder !== 0) return byOrder
  if (a.name === PERSONA_PREFIX_SECTION) return -1
  if (b.name === PERSONA_PREFIX_SECTION) return 1
