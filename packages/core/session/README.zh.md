...
Responses 风格 adapter 可以在普通 message stream 旁记录 `llm/response-opened`、有序的 `llm/response-item` 和 `llm/response-closed` 事件。事件 payload 会把每个 item 与终态绑定到一条 provider/model 路由和 response id；adapter 代码必须在追加前移除凭据和原始传输细节。`applyLlmResponseReplay()` 会在重启后回放持久化序列，并在路由变化、response id 变化、序号空洞或生命周期违规时以 `INVALID_REPLAY_STATE` 拒绝。
