...
### Responses replay events

Responses-style adapters may record `llm/response-opened`, ordered `llm/response-item`, and `llm/response-closed` events alongside the ordinary message stream. The event payload binds every item and terminal status to one provider/model route and response id; adapter code must remove credentials and raw transport details before append. `applyLlmResponseReplay()` replays the durable sequence after restart and rejects route changes, response-id changes, gaps, and lifecycle violations with `INVALID_REPLAY_STATE`.
