...
function difficultyGuidance(difficulty: BenchDifficulty): string {
  switch (difficulty) {
    case 'easy':
      return '简单：约 8 分钟单模型工作量；聚焦一个核心能力点，要求明确，边界数量少但仍可客观判定。'
    case 'hard':
      return '困难：约 20 分钟单模型工作量；包含多步骤推理或实现、多个相互作用的约束及关键边界，必须有足够区分度但仍可在限时内完成。'
    default:
      return '中等：约 15 分钟单模型工作量；包含两个以上相关要求和必要边界，能区分基础完成与高质量完成。'
  }
}
...
function fileContract(difficulty: BenchDifficulty): string {
  return [
    ...
    `3. meta.json —— 严格 JSON：{"title": string(20字内题目标题), "difficulty": "${difficulty}", "passThreshold": number(0-10 的通过分数线，默认 6), "judgeNotes": string(给裁判的额外评分提示)}；difficulty 必须与用户选择完全一致。`,
...
export function generatorPrompt(category: BenchCategory, difficulty: BenchDifficulty = 'medium', visionImage?: string, paperTopicHint?: string): string {
  const head = [
    ...
    '要求：题目自洽、可判定，严格匹配所选难度；不得要求被试模型访问付费资源或进行破坏性操作。',
  ].join('\n')
  const body = (() => {
    switch (category) {
      case 'coding':
        return [
          ...
          '- 在 answer.md 中给出参考实现，以及至少 3 组测试用例（含 1 组边界用例）与预期输出；裁判将实际运行被试代码验证。',
