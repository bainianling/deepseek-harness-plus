...
| `modelEnvironmentPolicy` | `basic` / `foreground` / `sequential` | 新建 Session 默认使用的压缩、后台任务与工具调用并发策略 |
...
`session/enhancePrompt` 使用目标会话当前选中的模型改写一段草稿提示词。请求必须携带与该会话一致的完整选择（provider、model、推理等级），否则 Host 以过期选择为由拒绝。当 `readProject` 为真时，Host 先构建一份有界的只读工作区摘要（按深度与体量封顶、文本扩展名白名单、跳过密钥文件名、拒绝符号链接），并作为上下文提供给模型；摘要不会持久化。该调用是严格的一次性请求：仅接受纯文本输出且有硬性 token 上限，任何工具调用、推理或错误结束都会被拒绝，并有 90 秒超时。失败使用 `session/prompt-enhancement-{invalid,unavailable,failed}` 错误码族；原始草稿在 Host 端不会被修改。

#### KV Cache 影响

无；一次性插件调用与会话历史隔离，不会改变组装后的请求或提供方缓存输入。
...
- 模型环境计划目前只是持久化元数据。本阶段不会启用 OpenAI Responses 回放、provider-managed continuation、后台 job、native compaction 或按路由选择执行 adapter。
