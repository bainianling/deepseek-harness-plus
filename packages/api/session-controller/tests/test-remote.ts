...
export interface TestSessionRemote {
  ...
  selectModelRoles(request: SessionSelectModelRolesRequest): Promise<RemoteResult<SessionSelectModelRolesValue>>
  modelCatalog(): Promise<RemoteResult<ModelCatalog>>
  modelEnvironment(request: SessionModelEnvironmentRequest): Promise<RemoteResult<SessionModelEnvironmentValue>>
  enhancePrompt(request: PromptEnhancementRequest, signal?: AbortSignal): Promise<RemoteResult<PromptEnhancementValue>>
...
export function createSessionTestRemote(
  ...
  return {
    ...
    selectModelRoles: request => remoteResult(() => direct.selectModelRoles(request)),
    modelCatalog: () => remoteResult(() => direct.modelCatalog()),
    modelEnvironment: request => remoteResult(() => direct.modelEnvironment(request)),
    enhancePrompt: (request, signal = new AbortController().signal) => remoteResult(
      () => direct.enhancePrompt(request, signal),
      signal,
    ),
