...
describe('ApiSession create or adoption', () => {
  it('records the initial model environment before Agent publication', async () => {
    const { ctx, agents } = await harness()
    ctx.provide('llm', {
    ...
    const cwd = mkdtempSync(join(tmpdir(), 'dsh-session-controller-environment-'))
    const meta = header('environment-before-publication', cwd)
    const created = agent(ctx, meta)
    const agentCtx = ctx.extend({ agent: created })
    vi.spyOn(ctx.agents, 'create').mockImplementation(async (options) => {
      await options.setup?.(agentCtx, created)
      return { agent: created, dispose: () => Promise.resolve() }
    })

    await expect(agents.ensureSession(meta.id, cwd, false)).resolves.toBe(created)

    expect(created.session.snapshotEvents()).toEqual(expect.arrayContaining([
