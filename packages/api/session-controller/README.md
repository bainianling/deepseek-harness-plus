...
| `modelEnvironmentPolicy` | `basic` / `foreground` / `sequential` | Default compaction, background, and tool-call concurrency choices for newly created Sessions |
...
`session/enhancePrompt` rewrites one draft prompt with the model currently selected by the addressed Session. The request must carry that exact selection — provider, model, and reasoning effort — and the Host rejects anything else as stale. When `readProject` is set, the Host first builds a bounded, read-only workspace summary (depth- and size-capped, text-allowlisted, secret filenames skipped, symlinks refused) and offers it to the model as context; the summary is never persisted. The call is a strict one-shot: text-only output at a hard token cap, any tool-call, reasoning, or error finish rejected, and a 90-second timeout. Failures use the `session/prompt-enhancement-{invalid,unavailable,failed}` code family; the original draft is never mutated on the Host.

#### KV Cache effect

None; one-shot plugin calls are isolated from session history and do not change the assembled request or provider cache inputs.
...
- The environment plan is durable metadata only. This stage does not enable OpenAI Responses replay, provider-managed continuation, background jobs, native compaction, or route-specific execution adapters.
