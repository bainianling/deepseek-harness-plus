...
The Session face also exposes six terminal operations: `terminalList`, `terminalOpen`, `terminalSend`, `terminalRead`, `terminalSignal`, and `terminalClose`. They resolve the addressed Agent before reaching the Host PTY registry, so terminal authority remains exact-Agent scoped. Web-created PTYs use an owner-local `web:` name prefix; the browser sees only those sessions and cannot operate the persistent shell sessions used by model tools. `terminalSend` returns after input is accepted; bounded scrollback and the `busy` snapshot field are the polling contract for command completion. The surface is line-oriented and does not emulate raw xterm bytes, resize, ANSI screen state, or full-screen TUI behavior.
