...
本包提供 Web GUI 的外壳布局：一个三栏 AppFrame，带可缩放的侧栏与详情面板；一条让步链，在空间不足时先收缩详情栏、随后自动关闭它；以及 `ctx.layout` 面板几何服务，供其他插件调用以打开或关闭详情栏。它还承载主题呈现器，把解析后的配色方案、别名 token、正文字号与 `theme-color` 元数据投影到 document。Terminal 栏是按 Session 隔离的行式 PTY 工作区：通过 Session face 创建 `web:` 终端，轮询有界 scrollback，支持命令历史与 SIGINT，并在导航时保持 Host 进程存活；它不承诺 raw xterm、ANSI 屏幕恢复、resize 或全屏 TUI。需要标准窗口外观时选择它；面板几何是瞬时的，重新加载即重置。
