...
describe('workspace browser rows', () => {
  it('omits only an empty leading status slot in the hierarchy-free flat list', () => {
    ...
    const view = render(<SessionNodeItem node={idle} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onMove={vi.fn()} onArchive={vi.fn()} onDelete={vi.fn()} flat t={t} />)
    ...
    view.rerender(<SessionNodeItem node={{ ...idle, running: true }} currentId={undefined} now={0}
      onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onMove={vi.fn()} onArchive={vi.fn()} onDelete={vi.fn()} flat t={t} />)
  ...
  it('renders and opens a selected running Session row', () => {
    ...
    render(
      <SessionNodeItem node={node} currentId={node.id} now={0} onOpen={onOpen}
        onRename={vi.fn()} onFork={vi.fn()} onMove={vi.fn()} onArchive={vi.fn()} onDelete={vi.fn()} t={t} />,
    )
  ...
  it('keeps the active-Schedule marker between the title and time in grouped and flat rows', () => {
    ...
    const view = render(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onMove={vi.fn()} onDelete={vi.fn()} t={t} />,
    )
    ...
    view.rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onMove={vi.fn()} onDelete={vi.fn()} flat t={t} />,
    )
    assertIndicator()
  })
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, move, archive, and delete', () => {
    const onOpen = vi.fn()
    const onRename = vi.fn()
    const onFork = vi.fn()
    const onMove = vi.fn()
    ...
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
      onRename={onRename} onFork={onFork} onMove={onMove} onArchive={onArchive} onDelete={onDelete} t={t} />)
    ...
    // Delete removes the stored log with no undo: danger styling.
    expect(screen.getByRole('menuitem', { name: '删除对话' }).className).toMatch(/danger/)
    ...
    // Move dispatches with the current display title (dialog prefill).
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '移动到工作区…' }))
    expect(onMove).toHaveBeenCalledWith(node.id, 'One')
    // Archive dispatches without opening the session.
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '归档会话' }))
    expect(onArchive).toHaveBeenCalledWith(node.id)
    // Delete dispatches with the current display title (confirmation prefill).
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
  ...
  it.each([
    ...
    try {
      ...
      view.rerender(<SessionNodeItem node={{ ...node, running: false }} currentId={undefined} now={0}
        onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onMove={vi.fn()} onArchive={vi.fn()} onDelete={vi.fn()} t={t} />)
  ...
  it('draggable row wires start/end and gates hover/drop on an active same-group drag', () => {
    ...
    const { rerender } = render(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onMove={vi.fn()} onArchive={vi.fn()} onDelete={vi.fn()} drag={inactive} t={t} />,
    )
    const row = screen.getByRole('treeitem')
    stubRect(row)
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onMove={vi.fn()} onArchive={vi.fn()} onDelete={vi.fn()} drag={active} t={t} />,
    )
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onMove={vi.fn()} onArchive={vi.fn()} onDelete={vi.fn()} drag={after} t={t} />,
    )
