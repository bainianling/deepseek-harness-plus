...
composer 的「丰富」入口紧贴模型位左侧。展开后显示一个内嵌选项——宿主是否先读取一份有界的只读项目摘要——以及改写动作。请求携带目录当前的完整选择，因此宿主可以拒绝过期模型；被拒绝的调用以内联错误条呈现，草稿保持不变。改写被接受后原地替换草稿，并在面板内保留改写前文本供一步还原；草稿绝不会自动发送。
...
提示词丰富入口是同一份会话级目录上的第三个 `conversation.input.right` 占位：其注入面绑定目录存储与一个 `session.enhancePrompt` 闭包，组件通过标准 `useInput` 钩子读取草稿。它对已寻址 subagent 会话不渲染，按宿主的精确选择规则原样发送 `directory.current`，并把还原缓冲仅保存在组件状态里。
...
间接影响。两个入口都提交 `session.selectModel` 选择；宿主在下一次提示词组装边界对完整 `ModelSelection` 快照并拥有模型可见效果，而运行中的步骤保留已组装选择。提示词丰富入口读取同一份目录，并把其确切的 `current` 作为一次性改写路由发送；宿主在调用模型前会对照会话的实时选择校验。

#### KV Cache 影响

切换路由可能减少提供方侧后续请求的缓存复用，或使其失效；提示词前缀本身不受影响。提示词丰富的一次性调用与会话历史隔离，对两者都无影响。
...
- **无创建期或已寻址 subagent 选择**——所有入口都要求既有普通会话的 agent；没有可纳入会话创建的草稿阶段模型选择，subagent 继续执行也有意不公开独立的模型选择或提示词丰富约定。
