...
No additional model-visible content; this entry describes route metadata returned beside the request.

#### Token effect

No direct token effect; capability metadata is not serialized into a provider request.

#### KV Cache effect

No direct cache effect; the metadata describes the adapter and does not change request dispatch.
