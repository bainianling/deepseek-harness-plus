...
describe('hand-declared providers', () => {
  ...
  it('describes the native protocol while keeping provider-managed state opt-in', async () => {
    const server = await mockServer([])
    const ctx = await harness({
      providers: {
        'acme-responses': {
          ...
          cacheRetention: 'long',
          models: [{ id: 'acme-responses', contextWindow: 65_536, maxTokens: 4096 }],
        },
      },
    })

    await expect(ctx.llm.resolveModelInfo('acme-responses', 'acme-responses'))
