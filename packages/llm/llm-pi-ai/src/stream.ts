...
import { CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, isContextWindowExceededError, isQuotaExceededError, LlmError, ProviderResponseId, QUOTA_EXCEEDED_CODE } from '@deepseek-ai/dsh-llm'
import type { FinishReason, LlmResponseItemKind, LlmResponseLifecycle, StreamChunk, TokenUsage, ToolCallId } from '@deepseek-ai/dsh-llm'
import type { JsonValue } from '@deepseek-ai/dsh-util-values'
import { isContextOverflow } from '@earendil-works/pi-ai'
import type { AssistantMessage, AssistantMessageEvent, Usage as PiUsage } from '@earendil-works/pi-ai'
import { toPiReplayState } from './replay.ts'

/** Convert one pi-ai content block into the credential-free response vocabulary. */
function responseItemPayload(content: AssistantMessage['content'][number]): {
  kind: LlmResponseItemKind
  payload: JsonValue
} {
  switch (content.type) {
    case 'text':
      return { kind: 'message', payload: { type: 'output_text', text: content.text } }
    case 'thinking':
      return {
        kind: 'reasoning',
        payload: {
...
/** Build a route-bound lifecycle from one terminal pi-ai assistant message. */
function toResponseReplay(
  message: AssistantMessage,
  contextWindow: number | undefined,
  route?: { provider: string; model: string },
): LlmResponseLifecycle {
  const resolvedRoute = route ?? { provider: message.provider, model: message.model }
  const responseId = message.responseId === undefined ? undefined : ProviderResponseId(message.responseId)
  const opened = {
    route: resolvedRoute,
    state: 'client-replay' as const,
    ...responseId === undefined ? {} : { responseId },
  }
  const items = message.content.map((content, itemIndex) => {
    const item = responseItemPayload(content)
    return {
      route: resolvedRoute,
      itemIndex,
      ...responseId === undefined ? {} : { responseId },
  ...
  const reason = mapStopReason(message, contextWindow)
  const closed = reason.kind === 'aborted'
    ? { route: resolvedRoute, status: 'cancelled' as const, ...responseId === undefined ? {} : { responseId }, failure: reason.failure }
    : reason.kind === 'error'
      ? { route: resolvedRoute, status: 'failed' as const, ...responseId === undefined ? {} : { responseId }, failure: reason.failure }
      : reason.kind === 'max-tokens'
        ? { route: resolvedRoute, status: 'incomplete' as const, ...responseId === undefined ? {} : { responseId } }
        : { route: resolvedRoute, status: 'completed' as const, ...responseId === undefined ? {} : { responseId } }
  return { opened, items, closed }
}
    ...
    switch (event.type) {
      ...
      case 'done':
        ...
        yield {
          ...
          responseReplay: toResponseReplay(event.message, contextWindow, route),
        }
        return
      case 'error':
        ...
        yield {
          ...
          responseReplay: toResponseReplay(terminal, contextWindow, route),
        }
        return
