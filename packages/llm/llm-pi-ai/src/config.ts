...
export interface PiAiProviderProfile {
  ...
  /**
   * Repetition penalty injected into every OpenAI completions payload on this
   * route (default unset = the provider's own sampling). Counters degenerate
   * repetition loops on long sessions; values above 1 penalize repeated
   * tokens, and gateways accept (0, 2). Other wire protocols ignore it —
   * their gateways reject unknown fields, so it is never sent to them.
   */
