...
| [`src/stream.ts`](src/stream.ts) | pi-ai event conversion into harness `StreamChunk` values and redacted response lifecycle records |
...
No additional model-visible content; this entry describes pi-ai route metadata returned beside the request.

#### Token effect

No direct token effect; the metadata is not serialized into a provider request.

#### KV Cache effect

No direct cache effect; the metadata describes the installed adapter and does not switch protocol dispatch.
