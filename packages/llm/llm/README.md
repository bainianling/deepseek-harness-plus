...
### Responses replay lifecycle

The package exports the provider-neutral `llm/response-opened`, `llm/response-item`, and `llm/response-closed` event vocabulary and its pure replay reducer. Adapters append only credential-free, provider-validated JSON payloads; the reducer preserves response ids, exact provider/model routes, item order, reasoning and tool items, and completed, incomplete, failed, or cancelled terminal status. Provider-managed continuation and client replay selection remain adapter responsibilities.
...
| [`src/response-replay.ts`](src/response-replay.ts) | Provider-neutral Responses lifecycle events and immutable replay reducer |
...
- **Responses replay is route-bound** — `response-item` and `response-closed` records must match the opened provider/model route, response id, and contiguous item order; invalid state fails with `INVALID_REPLAY_STATE`.
...
None, as the LLM service forwards assembled requests and exposes adapter-owned model metadata without directly modifying model context.
...
- **The Responses lifecycle is vocabulary only** — the OpenAI Responses wire adapter, provider-managed continuation, and native compaction remain deferred until their producers and SDK snapshots land.
