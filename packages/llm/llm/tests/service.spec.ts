...
describe('LlmRuntime', () => {
  ...
  it('detaches exact model execution capabilities', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    const source: LlmModelCapabilities = {
    ...
    const adapter = new class extends ScriptedAdapter {
      override resolveModel(): Promise<LlmResolvedModelInfo> {
        return Promise.resolve({ provider: 'route', id: 'model', name: 'Model', capabilities: source })
      }
    }(SCRIPT)
    ctx.llm.registerAdapter(['route'], adapter)

    const resolved = await ctx.llm.resolveModelInfo('route', 'model')
    expect(resolved.capabilities).toEqual(source)
    source.protocol = 'changed'
    expect(resolved.capabilities?.protocol).toBe('openai-responses')
  })

  it.each([
    { protocol: '', state: 'client-replay', promptCaching: 'none', nativeCompaction: false, background: false, parallelToolCalls: false },
    { protocol: 'x', state: 'unknown', promptCaching: 'none', nativeCompaction: false, background: false, parallelToolCalls: false },
    { protocol: 'x', state: 'client-replay', promptCaching: 'unknown', nativeCompaction: false, background: false, parallelToolCalls: false },
    { protocol: 'x', state: 'client-replay', promptCaching: 'none', nativeCompaction: 'yes', background: false, parallelToolCalls: false },
  ] as const)('rejects invalid exact model capabilities', async (capabilities) => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    const adapter = new class extends ScriptedAdapter {
      override resolveModel(): Promise<LlmResolvedModelInfo> {
        return Promise.resolve({
          provider: 'route', id: 'model', name: 'Model',
          capabilities: capabilities as unknown as LlmModelCapabilities,
        })
      }
    }(SCRIPT)
    ctx.llm.registerAdapter(['route'], adapter)
    ...
    await expect(ctx.llm.resolveModelInfo('route', 'model'))
      .rejects.toMatchObject({ code: 'INVALID_MODEL_CAPABILITIES' })
  })
