...
import type { LlmResponseLifecycle } from './response-replay.ts'
...
/** Execution state strategy currently implemented by an adapter. */
export type LlmModelState = 'client-replay' | 'provider-managed'

/** Prompt-cache ownership currently implemented by an adapter. */
export type LlmModelPromptCaching = 'none' | 'provider'

/** Protocol and execution features for one exact provider/model route. */
export interface LlmModelCapabilities {
  /** Wire protocol used by the adapter for this model. */
  protocol: string
  /** Whether conversation state is replayed by the harness or owned by the provider. */
  state: LlmModelState
  /** Whether the adapter currently routes a stable prefix to provider caching. */
  promptCaching: LlmModelPromptCaching
  /** Whether the adapter persists and replays a provider-native compaction item. */
  nativeCompaction: boolean
  /** Whether the adapter can run this model through a background request lifecycle. */
  background: boolean
  /** Whether the adapter can preserve multiple tool calls from one model turn. */
...
export interface LlmResolvedModelInfo extends LlmModelInfo {
  ...
  /** Adapter-owned execution features for this exact route. */
...
export type StreamChunk =
  ...
  | {
    ...
    /** Provider-neutral response lifecycle, when the adapter can emit one. */
    responseReplay?: LlmResponseLifecycle
  }
