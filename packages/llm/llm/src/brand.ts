...
/** Provider-issued response identifier retained for durable replay. */
export type ProviderResponseId = Branded<'ProviderResponseId'>

/**
 * Brand a provider response identifier.
 * @param id - the opaque provider-issued response id.
 * @returns the same string with the response-id brand.
 */
export function ProviderResponseId(id: string): ProviderResponseId {
  return brandString<ProviderResponseId>(id)
}
