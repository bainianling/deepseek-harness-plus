...
export * from './response-replay.ts'
...
export class LlmRuntime extends TypertRemoteService {
  ...
  private normalizeModelInfo(
    ...
    if (capabilities !== undefined) {
      if (
        typeof capabilities.protocol !== 'string'
        || capabilities.protocol.length === 0
        || (capabilities.state !== 'client-replay' && capabilities.state !== 'provider-managed')
        || (capabilities.promptCaching !== 'none' && capabilities.promptCaching !== 'provider')
        ...
        throw new LlmError(
          `adapter returned invalid capability metadata for provider "${provider}" model "${model}"`,
          'INVALID_MODEL_CAPABILITIES',
        )
      }
      const detachedCapabilities: LlmModelCapabilities = {
      ...
      info.capabilities = detachedCapabilities
    }
