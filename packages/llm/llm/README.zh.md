...
本包导出与 provider 无关的 `llm/response-opened`、`llm/response-item` 和 `llm/response-closed` 事件词汇及其纯回放 reducer。Adapter 只追加经过 provider 校验且不含凭据的 JSON payload；reducer 保留 response id、精确的 provider/model 路由、item 顺序、reasoning 与 tool item，以及 completed、incomplete、failed 或 cancelled 终态。Provider-managed continuation 与 client replay 的选择仍由 adapter 负责。
...
| [`src/response-replay.ts`](src/response-replay.ts) | 与 provider 无关的 Responses 生命周期事件与不可变回放 reducer |
...
- **Responses 回放绑定路由**——`response-item` 和 `response-closed` 必须匹配 opened 事件的 provider/model 路由、response id 和连续 item 顺序；非法状态以 `INVALID_REPLAY_STATE` 失败。
...
没有直接影响，因为 LLM 服务只转发已组装请求，并在请求旁暴露由适配器拥有的模型元数据。
...
- **Responses 生命周期目前只有事件词汇**——OpenAI Responses wire adapter、provider-managed continuation 与 native compaction 会等到对应 producer 和 SDK snapshot 落地后再延期结束。
