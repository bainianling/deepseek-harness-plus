...
/**
 * Local-compat for profile-installed plugins built against the pre-alpha.2
 * settings API: mint a namespace the old way (validated identically).
 * @param value - lowercase hyphenated namespace literal.
...
/**
 * Local-compat for profile-installed plugins built against the pre-alpha.2
 * settings API: bridge the old install call onto {@link SettingsProvider.register}.
 * The resolved scope is mirrored into the caller's hooks (and optional entry
 * object) at attach and after every committed change; a missing or duplicate
 * settings service skips the wiring, leaving the composed policy.
 * @param ctx - consumer plugin context.
 * @param ns - namespace literal.
 * @param schema - schemastery schema resolving the section.
 * @param entry - optional mutable mirror of the resolved value.
 * @param hooks - old-style source/onChange/validate hooks.
 */
export function installSettingsSection<T>(
  ctx: Context,
  ns: string,
  schema: z<T>,
  entry: Partial<T> | undefined,
  hooks: SettingsSectionHooks<T>,
): void {
  try {
    const scope = ctx.settings.register(ns, schema, hooks.validate === undefined ? {} : { validate: hooks.validate })
    hooks.setSource(() => scope.get())
    if (entry !== undefined) Object.assign(entry, scope.get())
    hooks.onChange()
    scope.watch(next => {
      if (entry !== undefined) Object.assign(entry, next)
      hooks.onChange()
    })
  } catch {
    // Settings service absent (other realm) or namespace taken: skip wiring.
  }
}
