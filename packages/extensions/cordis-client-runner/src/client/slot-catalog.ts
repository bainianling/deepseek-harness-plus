...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.input.image-import',
    kind: 'single',
    scope: 'session-maybe',
    summary: 'Optional image-file picker immediately after the command launcher in the composer tool row.',
    doc: 'Optional image-file picker immediately after the command launcher in the composer tool row.',
    registerOptions: [],
    ownerProps: [
      '/** Input state handed to the optional attachment presentation plugin. */\nexport interface ComposerAttachmentsOwnerProps {\n  /** Browser-owned draft images in input order. */\n  attachments: readonly ComposerAttachment[]\n  /** Whether a document-level file drop may add images now. */\n  canAcceptDrop: boolean\n  /** Add one dropped batch through the composer\'s validation path. */\n  onAddImages: (files: readonly File[]) => void\n  /** Remove one draft image through the Conversation service. */\n  onRemoveImage: (id: DraftAttachmentId) => void\n  /** Display-ready limits for the drop invitation. */\n  dropLimits?: { readonly count: number; readonly size: string } | undefined\n}',
    ],
    ...
    declaredBy: 'an entry in \'conversation.composer.bar\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-attachment ComposerImageImport',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  {
    key: 'conversation.input.jailbreak',
    kind: 'single',
    scope: 'session',
    summary: 'Jailbreak control inside the composer tool row.',
    doc: 'Jailbreak control inside the composer tool row.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'conversation.composer.bar\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-jailbreak JailbreakChip',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-settings-plugins CompactionCard',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-model-selection ModelBalanceAction id \'model-balance\'',
      'client-ui-settings-general StopServerAction id \'stop-server\'',
