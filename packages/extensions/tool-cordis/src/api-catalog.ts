...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'collabStudio',
    summary: 'The `collabStudio` service surface other rows may use.',
    description: 'The `collabStudio` service surface other rows may use.',
    methods: [
      {
        signature: 'listProjects(): Promise<unknown>',
        description: 'List persisted collaboration projects.',
        parameters: [],
        returns: 'persisted project records.',
      },
      {
        signature: 'createProject(request: unknown): Promise<ProjectRecord>',
        description: 'Create one collaboration project from a user request.',
        parameters: [{ name: 'request', description: 'untrusted project creation input.' }],
        returns: 'the created project record.',
      },
      {
        signature: 'getProject(projectId: string): Promise<unknown>',
        description: 'Read one project together with its role and stage metadata.',
        parameters: [{ name: 'projectId', description: 'persisted project identifier.' }],
        returns: 'project detail data.',
      },
      {
        signature: 'startProject(projectId: string): Promise<{ started: boolean; reason?: string }>',
        description: 'Start one project pipeline.',
        parameters: [{ name: 'projectId', description: 'persisted project identifier.' }],
        returns: 'whether the pipeline was started.',
      },
      {
        signature: 'stopProject(projectId: string): Promise<{ stopped: boolean }>',
        description: 'Stop one running project pipeline.',
        parameters: [{ name: 'projectId', description: 'persisted project identifier.' }],
        returns: 'whether an active pipeline was stopped.',
      },
      {
        signature: 'deleteProject(projectId: string): Promise<{ deleted: boolean }>',
        description: 'Delete one project after stopping any active run.',
        parameters: [{ name: 'projectId', description: 'persisted project identifier.' }],
        returns: 'whether the project was deleted.',
      },
      {
        signature: 'events(projectId: string, after: number, limit: number): Promise<{ events: unknown[]; total: number }>',
        description: 'Read a bounded project event page.',
        parameters: [{ name: 'projectId', description: 'persisted project identifier.' }, { name: 'after', description: 'exclusive event cursor.' }, { name: 'limit', description: 'maximum number of events.' }],
        returns: 'the event page and total count.',
      },
      {
        signature: 'files(projectId: string): Promise<string[]>',
        description: 'List files generated for one project.',
        parameters: [{ name: 'projectId', description: 'persisted project identifier.' }],
        returns: 'project-relative file paths.',
      },
      {
        signature: 'readFile(projectId: string, path: string): Promise<{ text: string; truncated: boolean }>',
        description: 'Read one bounded generated project file.',
        parameters: [{ name: 'projectId', description: 'persisted project identifier.' }, { name: 'path', description: 'project-relative file path.' }],
        returns: 'file text and truncation status.',
      },
      {
        signature: 'isRunning(projectId: string): boolean',
        description: 'Report whether one project pipeline is active.',
        parameters: [{ name: 'projectId', description: 'persisted project identifier.' }],
  ...
  {
    key: 'jailbreakMode',
    summary: '`ctx.jailbreakMode`: owns logged jailbreak state, applies and narrates selected state at step start, wraps claimed user messages while active, the `jailbreak:policy` section, and the `/jailbreak` command.',
    description: '`ctx.jailbreakMode`: owns logged jailbreak state, applies and narrates selected state at step start, wraps claimed user messages while active, the `jailbreak:policy` section, and the `/jailbreak` command. UIs observe committed flips through `session/event`; there is no live mirror.',
    methods: [
      {
        signature: 'get(agent: Agent): JailbreakState & { pending?: JailbreakState }',
        description: 'Read the logged jailbreak state and any selection awaiting the next accepted in-turn pre-step.',
        parameters: [{ name: 'agent', description: 'The agent to read.' }],
        returns: 'Current logged state plus a pending selection, when present.',
      },
      {
        signature: 'set(agent: Agent, active: boolean, strategy: string = foldJailbreakMode(agent.session.snapshotEvents()).strategy): \'committed\' | \'queued\' | \'cancelled\' | \'noop\'',
        description: 'Select whether jailbreak mode should be active, optionally switching the strategy in the same flip. Between turns the method appends the change immediately because no in-turn pre-step will run until another prompt starts a turn. The open-turn fold is the idle signal: agent status stays `running` through post-turn checkpointing, when no further in-turn pre-step runs. During an open turn the selection remains pending until the next accepted in-turn pre-step. Repeated selection of the current or already-pending state is a no-op.',
        parameters: [{ name: 'agent', description: 'The agent to switch.' }, { name: 'active', description: 'Whether jailbreak mode should be active.' }, { name: 'strategy', description: 'The strategy id to select (defaults to the current one).' }],
  ...
  {
    key: 'modelBench',
    summary: 'The `modelBench` service surface other rows may use.',
    description: 'The `modelBench` service surface other rows may use.',
    methods: [
      {
        signature: 'listRounds(): Promise<unknown>',
        description: 'List persisted benchmark rounds.',
        parameters: [],
        returns: 'persisted benchmark round records.',
      },
      {
        signature: 'createRound(request: unknown): Promise<RoundRecord>',
        description: 'Create one benchmark round from a user request.',
        parameters: [{ name: 'request', description: 'untrusted round creation input.' }],
        returns: 'the created benchmark round.',
      },
      {
        signature: 'getRound(roundId: string): Promise<unknown>',
        description: 'Read one round together with its question material.',
        parameters: [{ name: 'roundId', description: 'persisted round identifier.' }],
        returns: 'round detail data.',
      },
      {
        signature: 'generateRound(roundId: string): Promise<{ started: boolean; reason?: string }>',
        description: 'Start question generation for one round.',
        parameters: [{ name: 'roundId', description: 'persisted round identifier.' }],
        returns: 'whether generation was started.',
      },
      {
        signature: 'startRound(roundId: string, request: unknown): Promise<{ started: boolean; reason?: string }>',
        description: 'Start contestant execution for one round.',
        parameters: [{ name: 'roundId', description: 'persisted round identifier.' }, { name: 'request', description: 'untrusted contestant and judge selection input.' }],
        returns: 'whether the contest was started.',
      },
      {
        signature: 'stopRound(roundId: string): Promise<{ stopped: boolean }>',
        description: 'Stop one running benchmark round.',
        parameters: [{ name: 'roundId', description: 'persisted round identifier.' }],
        returns: 'whether an active round was stopped.',
      },
      {
        signature: 'deleteRound(roundId: string): Promise<{ deleted: boolean }>',
        description: 'Delete one round after stopping any active run.',
        parameters: [{ name: 'roundId', description: 'persisted round identifier.' }],
        returns: 'whether the round was deleted.',
      },
      {
        signature: 'events(roundId: string, after: number, limit: number): Promise<{ events: unknown[]; total: number }>',
        description: 'Read a bounded round event page.',
        parameters: [{ name: 'roundId', description: 'persisted round identifier.' }, { name: 'after', description: 'exclusive event cursor.' }, { name: 'limit', description: 'maximum number of events.' }],
        returns: 'the event page and total count.',
      },
      {
        signature: 'files(roundId: string): Promise<string[]>',
        description: 'List files generated for one round.',
        parameters: [{ name: 'roundId', description: 'persisted round identifier.' }],
        returns: 'round-relative file paths.',
      },
      {
        signature: 'readFile(roundId: string, path: string): Promise<{ text: string; truncated: boolean }>',
        description: 'Read one bounded generated round file.',
        parameters: [{ name: 'roundId', description: 'persisted round identifier.' }, { name: 'path', description: 'round-relative file path.' }],
        returns: 'file text and truncation status.',
      },
      {
        signature: 'isRunning(roundId: string): boolean',
        description: 'Report whether one benchmark round is active.',
        parameters: [{ name: 'roundId', description: 'persisted round identifier.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: '@Remote(\'selectModelRoles\') selectModelRoles(request: SessionSelectModelRolesRequest): Promise<SessionSelectModelRolesValue>',
        description: 'Select the thinking and worker routes used by dual-model Sessions.',
        parameters: [{ name: 'request', description: 'Session identity, enable flag, and both exact routes.' }],
      ...
      {
        signature: '@Remote(\'modelEnvironment\') async modelEnvironment(request: SessionModelEnvironmentRequest): Promise<SessionModelEnvironmentValue>',
        description: 'Resolve a secret-free execution plan for one exact model route.',
        parameters: [{ name: 'request', description: 'exact route, Agent preset, task policy, and overrides.' }],
        returns: 'detached model environment plan with stable reason codes.',
      },
      {
        signature: '@Remote(\'enhancePrompt\') async enhancePrompt( request: PromptEnhancementRequest, signal: AbortSignal, ): Promise<PromptEnhancementValue>',
        description: 'Rewrite one draft with the model currently selected by the addressed Session. Project context is optional and remains bounded/read-only on the Host.',
        parameters: [{ name: 'request', description: 'Session, exact selected route, draft, and project-context choice.' }, { name: 'signal', description: 'cancellation owned by the Remote unary carrier.' }],
      ...
      {
        signature: '@Remote(\'terminalList\') terminalList(request: SessionTerminalListRequest): Promise<SessionTerminalListValue>',
        description: 'List shell PTYs owned by the addressed Session Agent.',
        parameters: [{ name: 'request', description: 'Session identity for the terminal listing.' }],
        returns: 'the current terminal snapshots.',
      },
      {
        signature: '@Remote(\'terminalOpen\') terminalOpen(request: SessionTerminalOpenRequest): Promise<SessionTerminalOpenValue>',
        description: 'Create one shell PTY rooted at the Session workspace.',
        parameters: [{ name: 'request', description: 'Session identity and terminal launch options.' }],
        returns: 'the created terminal snapshot.',
      },
      {
        signature: '@Remote(\'terminalSend\') terminalSend(request: SessionTerminalSendRequest): Promise<SessionTerminalSendValue>',
        description: 'Submit text to one Session-owned PTY and wait for its next boundary.',
        parameters: [{ name: 'request', description: 'Session identity, terminal identity, and input text.' }],
        returns: 'the terminal state observed after sending input.',
      },
      {
        signature: '@Remote(\'terminalRead\') terminalRead(request: SessionTerminalReadRequest): Promise<SessionTerminalReadValue>',
        description: 'Read one bounded page from a Session-owned PTY scrollback.',
        parameters: [{ name: 'request', description: 'Session identity, terminal identity, and read cursor.' }],
        returns: 'one bounded scrollback page and its cursor.',
      },
      {
        signature: '@Remote(\'terminalSignal\') terminalSignal(request: SessionTerminalSignalRequest): Promise<SessionTerminalSignalValue>',
        description: 'Deliver one allowed signal to a Session-owned PTY foreground process group.',
        parameters: [{ name: 'request', description: 'Session identity, terminal identity, and signal name.' }],
        returns: 'the terminal state observed after delivering the signal.',
      },
      {
        signature: '@Remote(\'terminalClose\') terminalClose(request: SessionTerminalCloseRequest): Promise<SessionTerminalCloseValue>',
        description: 'Close one Session-owned PTY and await its process tree.',
        parameters: [{ name: 'request', description: 'Session identity and terminal identity.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'async delete(_id: SessionId, signal?: AbortSignal): Promise<void>',
        description: 'Durably remove one stored session: its events and its metadata cease to exist. Deleting an absent session is a no-op success, and a backend may reject deletion while a live writer still owns the session.',
        parameters: [{ name: '_id', description: 'the persisted session to delete.' }, { name: 'signal', description: 'optional cancellation for backend deletion work.' }],
      },
      {
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'guard(guard: (req: IncomingMessage) => boolean): () => void',
        description: 'Register a request guard before every route and fallback. A guard denying the request receives no application response, so a long-lived listener can withdraw LAN access without restarting its socket.',
        parameters: [{ name: 'guard', description: 'returns true only when this request may reach the route table.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: '@Remote(\'moveSession\') moveSession(request: WorkspaceMoveSessionRequest): Promise<WorkspaceValue>',
        description: 'Move one accounted Session into a target Workspace account.',
        parameters: [{ name: 'request', description: 'target Workspace, Session, and optional anchor identities.' }],
      ...
      },
      {
        ...
        description: 'Durably delete one Session from every Workspace account and persistence.',
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'async deleteSession(sessionId: SessionId): Promise<void>',
        description: 'Durably delete one session: remove it from the global archive set, from every workspace\'s accounting, and from session persistence itself (its stored events cease to exist). A LIVE session rejects — its owner fiber still appends to the log, and a concurrent append would re-materialize a partial artifact; close it first. An unknown id rejects like archiving. The detach-before-delete order keeps every failure recoverable: if the persistence write fails, the session survives as an ungrouped row and the request can simply be retried.',
        parameters: [{ name: 'sessionId', description: 'The session to delete.' }],
      },
      {
        signature: 'async moveSession( sessionId: SessionId, toWorkspaceId: WorkspaceId, beforeSessionId?: SessionId, ): Promise<Workspace>',
        description: 'Move one session into a target workspace\'s account, detaching it from every other account first — the one-owner invariant holds across the whole move because both halves ride one serialized operation.\n\nUnlike attach, membership in the target does not require the session\'s canonical cwd to equal the target path: an explicit move is user intent, so the target records the id as adopted and cwd-derived filtering never prunes it. A move within one workspace is still valid — detach from it, adopt back with the requested anchor — and reduces to a reorder. The anchor must already be accounted by the TARGET (it names where the session lands), which is why validation runs against that account even for a same-workspace move.',
        parameters: [{ name: 'sessionId', description: 'The live or persisted session to move.' }, { name: 'toWorkspaceId', description: 'Target workspace registration.' }, { name: 'beforeSessionId', description: 'Target-accounted anchor; omitted appends.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'BenchCategory',
    declaration: 'export type BenchCategory = \'coding\' | \'document\' | \'vision\' | \'paper\';',
  },
  {
    name: 'BenchDifficulty',
    declaration: 'export type BenchDifficulty = \'easy\' | \'medium\' | \'hard\';',
  },
  {
    name: 'Branded',
  ...
  {
    name: 'ContestantRecord',
    declaration: 'export interface ContestantRecord {\n    readonly slug: string;\n    readonly provider: string;\n    readonly model: string;\n    status: ContestantStatus;\n    startedAt?: number;\n    finishedAt?: number;\n    durationMs?: number;\n    replyTail?: string;\n    verdict?: JudgeVerdict;\n    error?: string;\n}',
  },
  {
    name: 'ContestantStatus',
    declaration: 'export type ContestantStatus = \'pending\' | \'running\' | \'done\' | \'failed\' | \'timeout\' | \'skipped\';',
  },
  {
    name: 'ContextFormed',
  ...
  {
    name: 'JailbreakState',
    declaration: 'export interface JailbreakState {\n    active: boolean;\n    strategy: string;\n}',
  },
  {
  ...
  {
    name: 'JudgeVerdict',
    declaration: 'export interface JudgeVerdict {\n    readonly pass: boolean;\n    readonly score: number | null;\n    readonly comment: string;\n}',
  },
  {
    name: 'KnobState',
  ...
  {
    name: 'LlmModelCapabilities',
    declaration: 'export interface LlmModelCapabilities {\n    protocol: string;\n    state: LlmModelState;\n    promptCaching: LlmModelPromptCaching;\n    nativeCompaction: boolean;\n    background: boolean;\n    parallelToolCalls: boolean;\n}',
  },
  {
  ...
  {
    name: 'LlmModelPromptCaching',
    declaration: 'export type LlmModelPromptCaching = \'none\' | \'provider\';',
  },
  {
  ...
  {
    name: 'LlmModelState',
    declaration: 'export type LlmModelState = \'client-replay\' | \'provider-managed\';',
  },
  {
  ...
  {
    name: 'ModelEnvironmentBackground',
    declaration: 'export type ModelEnvironmentBackground = \'foreground\' | \'durable\';',
  },
  {
    name: 'ModelEnvironmentCompaction',
    declaration: 'export type ModelEnvironmentCompaction = \'basic\' | \'native\' | \'disabled\';',
  },
  {
    name: 'ModelEnvironmentOverrides',
    declaration: 'export interface ModelEnvironmentOverrides {\n    readonly protocol?: string;\n    readonly state?: \'client-replay\' | \'provider-managed\';\n    readonly compaction?: ModelEnvironmentCompaction;\n    readonly background?: ModelEnvironmentBackground;\n    readonly parallelToolCalls?: boolean;\n}',
  },
  {
    name: 'ModelEnvironmentPlan',
    declaration: 'export interface ModelEnvironmentPlan {\n    readonly route: {\n        readonly provider: string;\n        readonly model: string;\n    };\n    readonly preset: string;\n    readonly protocol: string;\n    readonly state: \'client-replay\' | \'provider-managed\';\n    readonly promptCaching: \'none\' | \'provider\';\n    readonly compaction: ModelEnvironmentCompaction;\n    readonly background: ModelEnvironmentBackground;\n    readonly parallelToolCalls: boolean;\n    readonly reasons: readonly ModelEnvironmentReason[];\n}',
  },
  {
    name: 'ModelEnvironmentPolicy',
    declaration: 'export interface ModelEnvironmentPolicy {\n    readonly compaction: ModelEnvironmentCompaction;\n    readonly background: ModelEnvironmentBackground;\n    readonly parallelToolCalls: boolean;\n}',
  },
  {
    name: 'ModelEnvironmentReason',
    declaration: 'export interface ModelEnvironmentReason {\n    readonly field: string;\n    readonly source: \'route\' | \'preset\' | \'task\' | \'override\' | \'fallback\';\n    readonly code: string;\n}',
  },
  {
  ...
  {
    name: 'ProjectStatus',
    declaration: 'export type ProjectStatus = \'draft\' | \'running\' | \'done\' | \'failed\' | \'stopped\';',
  },
  {
    name: 'ProjectType',
    declaration: 'export type ProjectType = \'software\' | \'webpage\' | \'document\' | \'research\' | \'other\';',
  },
  {
    name: 'PromptAssembly',
  ...
  },
  {
    ...
    declaration: 'export interface PromptEnhancementRequest extends ModelSelection {\n    readonly sessionId: SessionId;\n    readonly prompt: string;\n    readonly readProject: boolean;\n}',
  },
  {
    name: 'PromptEnhancementValue',
    declaration: 'export interface PromptEnhancementValue {\n    readonly prompt: string;\n}',
  },
  {
  ...
  {
    name: 'SessionModelEnvironmentRequest',
    declaration: 'export interface SessionModelEnvironmentRequest {\n    readonly provider: string;\n    readonly model: string;\n    readonly agentPreset?: string;\n    readonly policy: ModelEnvironmentPolicy;\n    readonly overrides?: ModelEnvironmentOverrides;\n}',
  },
  {
    name: 'SessionModelEnvironmentValue',
    declaration: 'export interface SessionModelEnvironmentValue {\n    readonly plan: ModelEnvironmentPlan;\n}',
  },
  {
  ...
  {
    name: 'SessionSelectModelRolesRequest',
    declaration: 'export interface SessionSelectModelRolesRequest {\n    readonly sessionId: SessionId;\n    readonly enabled: boolean;\n    readonly thinking: ModelSelection;\n    readonly worker: ModelSelection;\n}',
  },
  {
    name: 'SessionSelectModelRolesValue',
    declaration: 'export interface SessionSelectModelRolesValue {\n    readonly roles: ModelRoles;\n}',
  },
  {
  ...
  {
    name: 'SessionTerminalCloseRequest',
    declaration: 'export interface SessionTerminalCloseRequest {\n    readonly sessionId: SessionId;\n    readonly terminalSessionId: string;\n}',
  },
  {
    name: 'SessionTerminalCloseValue',
    declaration: 'export interface SessionTerminalCloseValue {\n    readonly closed: boolean;\n}',
  },
  {
    name: 'SessionTerminalListRequest',
    declaration: 'export interface SessionTerminalListRequest {\n    readonly sessionId: SessionId;\n}',
  },
  {
    name: 'SessionTerminalListValue',
    declaration: 'export interface SessionTerminalListValue {\n    readonly sessions: readonly SessionTerminalSnapshot[];\n}',
  },
  {
    name: 'SessionTerminalOpenRequest',
    declaration: 'export interface SessionTerminalOpenRequest {\n    readonly sessionId: SessionId;\n    readonly name?: string;\n}',
  },
  {
    name: 'SessionTerminalOpenValue',
    declaration: 'export interface SessionTerminalOpenValue extends SessionTerminalSnapshot {\n    readonly motd: string;\n}',
  },
  {
    name: 'SessionTerminalReadRequest',
    declaration: 'export interface SessionTerminalReadRequest {\n    readonly sessionId: SessionId;\n    readonly terminalSessionId: string;\n    readonly offset?: number;\n    readonly count?: number;\n}',
  },
  {
    name: 'SessionTerminalReadValue',
    declaration: 'export interface SessionTerminalReadValue {\n    readonly text: string;\n    readonly totalLines: number;\n    readonly lineBegin: number;\n    readonly lineEnd: number;\n    readonly truncated: boolean;\n}',
  },
  {
    name: 'SessionTerminalSendRequest',
    declaration: 'export interface SessionTerminalSendRequest {\n    readonly sessionId: SessionId;\n    readonly terminalSessionId: string;\n    readonly text: string;\n    readonly submit?: boolean;\n}',
  },
  {
    name: 'SessionTerminalSendValue',
    declaration: 'export interface SessionTerminalSendValue {\n    readonly accepted: true;\n}',
  },
  {
    name: 'SessionTerminalSignal',
    declaration: 'export type SessionTerminalSignal = \'SIGINT\' | \'SIGTERM\' | \'SIGKILL\' | \'SIGTSTP\' | \'SIGHUP\';',
  },
  {
    name: 'SessionTerminalSignalRequest',
    declaration: 'export interface SessionTerminalSignalRequest {\n    readonly sessionId: SessionId;\n    readonly terminalSessionId: string;\n    readonly signal: SessionTerminalSignal;\n}',
  },
  {
    name: 'SessionTerminalSignalValue',
    declaration: 'export interface SessionTerminalSignalValue {\n    readonly delivered: true;\n    readonly targetPgid: number;\n}',
  },
  {
    name: 'SessionTerminalSnapshot',
    declaration: 'export interface SessionTerminalSnapshot {\n    readonly sessionId: string;\n    readonly name?: string;\n    readonly type: string;\n    readonly pid?: number;\n    readonly status: SessionTerminalStatus;\n    readonly busy: boolean;\n}',
  },
  {
    name: 'SessionTerminalStatus',
    declaration: 'export type SessionTerminalStatus = {\n    readonly kind: \'running\';\n} | {\n    readonly kind: \'exited\';\n    readonly exitCode: number | null;\n    readonly signal: string | null;\n};',
  },
  {
  ...
  {
    name: 'StageStatus',
    declaration: 'export type StageStatus = \'pending\' | \'meeting\' | \'producing\' | \'reviewing\' | \'done\' | \'failed\' | \'skipped\';',
  },
  {
    name: 'StorageBackend',
