...
const validPayloads: Readonly<Record<string, SessionFormatJsonValue>> = {
  ...
  'game-swap/state': { active: true, to: 'another-save-dir', mode: 'prose' },
  'goal/change': {
  ...
  'jailbreak/mode': {
    active: true, strategy: 'authorized-ctf', strategyVersion: '1', strategySource: 'builtin',
  },
  'llm/response-closed': {
    route: { provider: 'mock', model: 'mock' }, status: 'failed', responseId: 'resp-1',
    failure: { message: 'Request timed out.', code: 'TIMEOUT' },
  },
  'llm/response-item': {
    route: { provider: 'mock', model: 'mock' }, itemIndex: 0, responseId: 'resp-1',
    kind: 'message', payload: { type: 'output_text', text: 'x' },
  },
  'llm/response-opened': {
    route: { provider: 'mock', model: 'mock' }, state: 'client-replay', responseId: 'resp-1',
  },
  'llm/retry': {
  ...
  'model/environment': {
    route: { provider: 'mock', model: 'mock' }, preset: 'zh', protocol: 'openai-completions',
    state: 'client-replay', promptCaching: 'provider', compaction: 'basic', background: 'foreground',
    parallelToolCalls: false,
    reasons: [{ field: 'protocol', source: 'route', code: 'route-protocol' }],
  },
  'model/selection': { provider: 'mock', model: 'mock', reasoningEffort: 'high' },
  'permission/preset': { preset: 'default', origin: 'default' },
...
describe('released event and payload inventory', () => {
  it('has an executable valid fixture for every frozen released-v0 event type', () => {
    ...
    expect(RELEASED_V0_EVENT_TYPES).toHaveLength(57)
    // Types outside the generated current read vocabulary. The fork-era
    // historical events (`game-swap/state`, `llm/response-*`) are declared in
    // `src/historical-events.ts`, so the read path admits migrated logs; the
    // remaining three have dedicated edges (`assistant/chunk` collapses into
    // `assistant/attempt`, the retired code-dispatch names are
    // ignorable-only through the v2-to-v3 edge).
