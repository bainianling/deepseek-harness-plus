...
export const RELEASED_V0_EVENT_DISPOSITIONS: Readonly<Record<string, ReleasedV0PayloadDisposition>> = Object.freeze({
  ...
  // Local-customization events. The fork's own builds predate the released v0
  // inventory and wrote these plugin-owned types into v0 logs, so the frozen
  // table carries their exact envelope for those historical artifacts.
  'game-swap/state': disposition(['active', 'to', 'mode']),
  ...
  'jailbreak/mode': disposition(['active', 'strategy'], ['strategyVersion', 'strategySource']),
  'llm/response-closed': disposition(['route', 'status'], ['responseId', 'failure']),
  'llm/response-item': disposition(
    ['route', 'itemIndex', 'responseId', 'kind', 'payload'],
    [],
    ['payload'],
  ),
  'llm/response-opened': disposition(['route', 'state'], ['responseId']),
  ...
  'model/environment': disposition(
    ['route', 'preset', 'protocol', 'state'],
    ['promptCaching', 'compaction', 'background', 'parallelToolCalls', 'reasons'],
  ),
