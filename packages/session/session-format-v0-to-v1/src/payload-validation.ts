...
export function assertReleasedPayloadSemantics(event: SessionFormatEvent, version: number): void {
  ...
  switch (event.type) {
    ...
    case 'feedback/record':
      ...
      return
    case 'game-swap/state':
      booleanValue(data['active'], `${label} active`)
      stringValue(data['to'], `${label} to`)
      stringValue(data['mode'], `${label} mode`)
      return
    case 'goal/change':
    ...
    case 'jailbreak/mode':
      booleanValue(data['active'], `${label} active`)
      nonEmptyString(data['strategy'], `${label} strategy`)
      if (data['strategyVersion'] !== undefined) stringValue(data['strategyVersion'], `${label} strategyVersion`)
      if (data['strategySource'] !== undefined) stringValue(data['strategySource'], `${label} strategySource`)
      return
    case 'llm/response-closed':
      releasedRouteValue(data['route'], `${label} route`)
      nonEmptyString(data['status'], `${label} status`)
      if (data['responseId'] !== undefined) nonEmptyString(data['responseId'], `${label} responseId`)
      if (data['failure'] !== undefined) llmFailureValue(data['failure'], `${label} failure`)
      return
    case 'llm/response-item':
      releasedRouteValue(data['route'], `${label} route`)
      countValue(data['itemIndex'], `${label} itemIndex`)
      nonEmptyString(data['responseId'], `${label} responseId`)
      nonEmptyString(data['kind'], `${label} kind`)
      return
    case 'llm/response-opened':
      releasedRouteValue(data['route'], `${label} route`)
      nonEmptyString(data['state'], `${label} state`)
      if (data['responseId'] !== undefined) nonEmptyString(data['responseId'], `${label} responseId`)
      return
    case 'llm/retry':
    ...
    case 'model/environment':
      ...
      nonEmptyString(data['protocol'], `${label} protocol`)
      nonEmptyString(data['state'], `${label} state`)
      if (data['promptCaching'] !== undefined) stringValue(data['promptCaching'], `${label} promptCaching`)
      if (data['compaction'] !== undefined) stringValue(data['compaction'], `${label} compaction`)
      if (data['background'] !== undefined) stringValue(data['background'], `${label} background`)
      if (data['parallelToolCalls'] !== undefined) booleanValue(data['parallelToolCalls'], `${label} parallelToolCalls`)
      if (data['reasons'] !== undefined) {
        arrayValue(data['reasons'], `${label} reasons`, (reason, reasonLabel) => {
          const item = exactRecord(reason, reasonLabel, ['field', 'source', 'code'])
          stringValue(item['field'], `${reasonLabel} field`)
          stringValue(item['source'], `${reasonLabel} source`)
          stringValue(item['code'], `${reasonLabel} code`)
        })
      }
      return
    ...
    case 'tool/call':
      ...
      // One fork-era artifact recorded an empty tool name; the historical
      // envelope stays log-only either way, so the name is only type-checked.
...
function releasedRouteValue(value: SessionFormatJsonValue | undefined, label: string): void {
...
function contentBlockValue(value: SessionFormatJsonValue, label: string, version: number): void {
  ...
  switch (block['type']) {
    ...
    case 'tool-call':
      ...
      // One fork-era artifact recorded an empty tool name in an assistant
      // message block; the name is only type-checked for those logs.
...
function subagentDescriptorValue(data: JsonRecord, label: string): void {
  // Version 2 is the fork's own pre-release descriptor generation; released v0
  // artifacts written by the customization line carry it alongside v3.
