...
/** Historical fork-era events the migration catalog preserves into v3. */
const forkEraRoute = { provider: 'pptoken', model: 'gpt-5.6-luna' }
const forkEraEvents = [
  { type: 'game-swap/state', seq: 1, time: 2, data: { active: true, to: 'stardew', mode: 'swap' } },
  { type: 'llm/response-opened', seq: 2, time: 3, data: { route: forkEraRoute, state: 'client-replay', responseId: 'resp_fork' } },
  {
    type: 'llm/response-item', seq: 3, time: 4,
    data: { route: forkEraRoute, itemIndex: 0, responseId: 'resp_fork', kind: 'message', payload: { type: 'output_text', text: 'hi' } },
  },
  { type: 'llm/response-closed', seq: 4, time: 5, data: { route: forkEraRoute, status: 'completed', responseId: 'resp_fork' } },
] as const
const forkEraPrefix = [header, start, ...forkEraEvents].map(row => JSON.stringify(row)).join('\n') + '\n'
...
describe('native V3 event admission at EOF', () => {
  ...
  it('admits the fork-era historical vocabulary through scan, read, and a provider append', async () => {
    const bytes = Buffer.from(forkEraPrefix)
    expect(scanLog(bytes)).toMatchObject({ events: [start, ...forkEraEvents], committedBytes: bytes.length })
    const path = await store(bytes)
    const sourceStat = await stat(path)
    const writer = await ctx.sessionPersistence.open(id, 'write')
    try {
      expect((await writer.read()).events).toEqual([start, ...forkEraEvents])
      await writer.append([{
    ...
    expect((await readFile(path)).subarray(0, bytes.length)).toEqual(bytes)
    expect(await stat(path)).toMatchObject({ ino: sourceStat.ino })
  })
