...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and publishes the URL with the LAN snapshot', async () => {
    ...
    apply(ctx, new Config({ openBrowser: true, printUrl: true, surfaceContext: true, trustedHosts: ['lab.internal'], hindsightUrl: 'http://127.0.0.1:9077', hindsightBankId: 'coding-agent::deepseek-harness', knowledgeTimeoutMs: 5000, hindsightProfile: 'dsh-local', hindsightCommand: 'hindsight-embed', hindsightStartTimeoutMs: 30_000 }))
  ...
  it('publishes no readiness side effect when printing and browser opening are disabled', async () => {
    ...
    internals.openBrowser = openBrowser
    apply(ctx, new Config({ openBrowser: false, printUrl: false, surfaceContext: true, trustedHosts: [], hindsightUrl: 'http://127.0.0.1:9077', hindsightBankId: 'coding-agent::deepseek-harness', knowledgeTimeoutMs: 5000, hindsightProfile: 'dsh-local', hindsightCommand: 'hindsight-embed', hindsightStartTimeoutMs: 30_000 }))
    await ctx.plugin(SystemPrompt, { personaPrefix: '' })
    await new Promise(resolve => setTimeout(resolve, 0))
  ...
  it('skips the surface context when disabled (the one-shot layer): no prompt section, no bash variables', async () => {
    ...
    apply(ctx, new Config({ openBrowser: false, printUrl: false, surfaceContext: false, trustedHosts: [], hindsightUrl: 'http://127.0.0.1:9077', hindsightBankId: 'coding-agent::deepseek-harness', knowledgeTimeoutMs: 5000, hindsightProfile: 'dsh-local', hindsightCommand: 'hindsight-embed', hindsightStartTimeoutMs: 30_000 }))
    await ctx.plugin(SystemPrompt, { personaPrefix: '' })
    await new Promise(resolve => setTimeout(resolve, 0))
    const assembly = await ctx.systemPrompt.assemble()
  ...
  it('prints the loopback-only URL line when no LAN snapshot exists', async () => {
    ...
    provideConnection(ctx)
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(ctx, new Config({ openBrowser: false, printUrl: true, surfaceContext: true, trustedHosts: [], hindsightUrl: 'http://127.0.0.1:9077', hindsightBankId: 'coding-agent::deepseek-harness', knowledgeTimeoutMs: 5000, hindsightProfile: 'dsh-local', hindsightCommand: 'hindsight-embed', hindsightStartTimeoutMs: 30_000 }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).toHaveBeenCalledWith('dsh web: http://127.0.0.1:4567/?token=test-token')
    await ctx.fiber.dispose()
  })
  ...
  it.each([
    ...
    const openBrowser = vi.fn(async () => {})
    internals.openBrowser = openBrowser
    apply(ctx, new Config({ openBrowser: true, printUrl: true, surfaceContext: false, trustedHosts: [], hindsightUrl: 'http://127.0.0.1:9077', hindsightBankId: 'coding-agent::deepseek-harness', knowledgeTimeoutMs: 5000, hindsightProfile: 'dsh-local', hindsightCommand: 'hindsight-embed', hindsightStartTimeoutMs: 30_000 }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).toHaveBeenCalledWith('dsh web: http://127.0.0.1:4567/?token=test-token')
    expect(openBrowser).not.toHaveBeenCalled()
    await ctx.fiber.dispose()
  ...
  it('defers readiness publication until Loader settlement and drops it on failure or teardown', async () => {
    ...
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(settled, new Config({ openBrowser: true, printUrl: true, surfaceContext: true, trustedHosts: [], hindsightUrl: 'http://127.0.0.1:9077', hindsightBankId: 'coding-agent::deepseek-harness', knowledgeTimeoutMs: 5000, hindsightProfile: 'dsh-local', hindsightCommand: 'hindsight-embed', hindsightStartTimeoutMs: 30_000 }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).not.toHaveBeenCalled()
    expect(openBrowser).not.toHaveBeenCalled()
    release!()
    ...
    apply(failed, new Config({ openBrowser: true, printUrl: true, surfaceContext: true, trustedHosts: [], hindsightUrl: 'http://127.0.0.1:9077', hindsightBankId: 'coding-agent::deepseek-harness', knowledgeTimeoutMs: 5000, hindsightProfile: 'dsh-local', hindsightCommand: 'hindsight-embed', hindsightStartTimeoutMs: 30_000 }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).not.toHaveBeenCalled()
    expect(openBrowser).not.toHaveBeenCalled()
    ...
    apply(torn, new Config({ openBrowser: true, printUrl: true, surfaceContext: true, trustedHosts: [], hindsightUrl: 'http://127.0.0.1:9077', hindsightBankId: 'coding-agent::deepseek-harness', knowledgeTimeoutMs: 5000, hindsightProfile: 'dsh-local', hindsightCommand: 'hindsight-embed', hindsightStartTimeoutMs: 30_000 }))
    await new Promise(resolve => setTimeout(resolve, 0))
  ...
  it.each([
    ...
    apply(ctx, new Config({ openBrowser: true, printUrl: false, surfaceContext: false, trustedHosts: [], hindsightUrl: 'http://127.0.0.1:9077', hindsightBankId: 'coding-agent::deepseek-harness', knowledgeTimeoutMs: 5000, hindsightProfile: 'dsh-local', hindsightCommand: 'hindsight-embed', hindsightStartTimeoutMs: 30_000 }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).toHaveBeenCalledWith('dsh web: opening the default browser; pass --no-open to disable')
