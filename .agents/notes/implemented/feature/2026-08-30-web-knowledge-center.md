...
The Web app owns a read-only `/api/knowledge` projection. `web-app` reads the configured `hindsightUrl` and `hindsightBankId` with a bounded `knowledgeTimeoutMs`, flattens the Hindsight knowledge tree, and exposes `/api/knowledge/snapshot` plus validated `/api/knowledge/pages/:id` reads. The snapshot carries source identity and status, folders, page summaries, source tags, fact/document/observation counts, consolidation timestamps, and pending or failed operation counts. An unavailable or timed-out source returns a structured `503` rather than hanging or fabricating an empty knowledge base. The separate [Hindsight manual-start decision](2026-09-04-hindsight-manual-start.md) owns when the local daemon is launched and how its lifecycle is surfaced.
