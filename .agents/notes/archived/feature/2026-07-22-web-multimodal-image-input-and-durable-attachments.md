...
Version one supports PNG, JPEG, WebP, and GIF paste, drag-and-drop, and file picking, image-only or mixed prompts, historical user and assistant image rendering, and original-image preview on a single click (display and interaction specifics superseded in part by the [attachment-display alignment note](2026-08-11-web-attachment-display-alignment.md)). Generic files, PDF, audio, video, image copying, and a custom context menu remain separate follow-ups.

### Product behavior

- Pasting, dropping, or choosing one or more supported images adds ordered pending-image entries above the textarea without inserting placeholder text. Each entry visibly presents a thumbnail, filename, and remove control, while the summary reports the number added. Dragging files over the composer highlights the drop target.
- A persistent attachment button in the same composer area opens a file picker limited to PNG, JPEG, WebP, and GIF. It delegates the selected batch to the same intake path as paste and drop.
- The same resident `InputBar` renders the attachment feedback in both blank-session Hero and active-session layouts. The attachment list scrolls horizontally instead of widening the composer.
- A prompt may contain text and images or images only. Pure text paste remains native browser behavior; mixed clipboard content inserts its text normally while adding its files to the attachment list, and file-only paste prevents default browser handling. File drops on the composer always prevent browser navigation and report unsupported files locally.
