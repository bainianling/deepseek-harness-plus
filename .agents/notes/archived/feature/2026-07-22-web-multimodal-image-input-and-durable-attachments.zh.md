...
第一版支持粘贴、拖放和文件选择 PNG、JPEG、WebP 与 GIF，支持仅图片或混合提示词，支持渲染历史用户图片与助手图片，并支持单击预览原图（展示与交互细节部分由[附件展示对齐 Note](2026-08-11-web-attachment-display-alignment.zh.md)取代）。通用文件、PDF、音频、视频、图片复制和自定义上下文菜单仍分别作为后续工作。

### 产品行为

- 粘贴、拖放或选择一张或多张受支持的图片后，文本框上方会按顺序显示待发送图片条目，但不会插入占位文本。每个条目明确显示缩略图、文件名和移除操作，汇总文本会显示已添加数量。文件拖入输入区时会高亮放置目标。
- 输入区内常驻的附件按钮会打开仅限 PNG、JPEG、WebP 与 GIF 的文件选择器。所选批次会进入与粘贴和拖放相同的接入流程。
- 同一个常驻 `InputBar` 会在空白会话 Hero 和活跃会话布局中渲染附件反馈区。附件列表通过横向滚动避免撑宽输入区。
- 提示词可同时包含文本与图片，也可仅包含图片。粘贴纯文本时保持浏览器原生行为；粘贴混合的剪贴板内容时，文本会正常插入，文件则同时添加到附件列表；仅粘贴文件时才阻止浏览器的默认处理。在输入区放置文件时总会阻止浏览器导航，并在本地报告不受支持的文件。
